...
| Fullscreen mouse interaction | Tool card/group click-to-toggle, previews, hover highlight, and a back-to-bottom button (motion reporting auto-enabled under tmux/zellij/screen) | `/ccstyle` (requires the official fullscreen TUI: set TUI mode in `/settings`, or launch with `--tui-mode fullscreen`) |
