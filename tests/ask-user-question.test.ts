...
test("questionnaire height preserves transcript space on normal terminals", () => {
	...
	assert.equal(
		getDialogMaxRows(24),
		17,
		"normal terminals reserve about 30% for transcript history",
	);
...
test("questionnaire uses the host's temporary editor flow in scrolling and fixed layouts", () => {
	const source = readFileSync(
		new URL("../extensions/ask-user-question/ask-user-question.ts", import.meta.url),
		"utf8",
	);
...
test("questionnaire is English-only without localization assets", () => {
	...
	assert.equal(
		existsSync(new URL("../extensions/ask-user-question/state/i18n-bridge.ts", import.meta.url)),
		false,
	);
	assert.equal(
		existsSync(new URL("../extensions/ask-user-question/locales", import.meta.url)),
		false,
	);
